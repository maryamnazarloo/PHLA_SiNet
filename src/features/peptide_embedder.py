...
class PeptideEmbedder:
    ...
    def _process_batch(self, batch: List[str]) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            batch_lens = mask.sum(1)  # Lengths of each sequence in the batch

            seq_reps = []
            for i, tokens_len in enumerate(batch_lens):
                # Extract sequence representations (average all tokens including first and last)
                seq_rep = token_representations[i, :tokens_len].mean(0)
    ...
    def embed_peptides(self, peptides: Union[List[str], pd.Series, pd.DataFrame],
        ...
        # all_embeddings1 = pd.DataFrame(all_embeddings)
        # all_embeddings1.to_csv('/content/a.csv', index=False)
